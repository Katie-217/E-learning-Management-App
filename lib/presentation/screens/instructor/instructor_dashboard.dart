...
class _InstructorDashboardState extends ConsumerState<InstructorDashboard> {
    ...
    try {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        Future.microtask(() {
          ...
          ref.invalidate(
              instructorAssignmentSubmissionStatsProvider(semesterName));
          ref.invalidate(instructorQuizCompletionStatsProvider(semesterName));
          ref.invalidate(instructorTasksForMonthProvider(
              InstructorTaskMonthKey(monthKey, semesterName)));
          ref.invalidate(instructorTasksForDateProvider(
    ...
    try {
      ...
      if (user != null) {
        ...
        if (doc.exists && mounted) {
          ...
          print(
              'DEBUG: Using Firebase Auth - name: $_userName, email: $_userEmail');
        }
      }
    } catch (e) {
          ...
          if (_selectedSemester == null && _semesters.isNotEmpty) {
            ...
            for (final semester in semesters) {
              if (semester.isCurrentSemester) {
                ...
                print(
                    'DEBUG: ✅ Found current semester: ${currentSemester.name}');
                break;
              }
            }
            ...
            if (currentSemester == null) {
              print(
                  'DEBUG: ⚠️ No current semester found, using first semester: ${_selectedSemester?.name}');
            }
  ...
  Future<void> _preloadDashboardDataWithSemester(String semesterName) async {
    ...
    Future.wait([
      ...
      ref.read(
          instructorAssignmentSubmissionStatsProvider(semesterName).future),
      ...
      ref.read(
          instructorTasksForDateProvider(InstructorTaskKey(now, semesterName))
              .future),
    ]).then((_) {
      if (mounted) {
        print(
            'DEBUG: ✅ Preloading dashboard data completed for semester: $semesterName');
  ...
  Widget build(BuildContext context) {
    ...
    final showBottomNav =
        !kIsWeb && !isWide; // chỉ dùng bottom nav cho mobile/app, tránh cho web
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: LayoutBuilder(
          builder: (context, constraints) {
            ...
            final iconSize =
                isVerySmall ? 28.0 : 32.0; // Giảm từ 32/40 xuống 28/32
            final titleSize =
                isVerySmall ? 13.0 : 15.0; // Giảm từ 14/16 xuống 13/15
                ...
                if (!isWide)
                  PopupMenuButton<String>(
                    ...
                    icon: Icon(Icons.menu,
                        color: Colors.white, size: isVerySmall ? 20.0 : 24.0),
                    ...
                    itemBuilder: (context) => const [
                      ...
                      PopupMenuItem(
                        ...
                        child: ListTile(
                          leading:
                              Icon(Icons.book_outlined, color: Colors.white70),
                          title: Text('Teaching',
                              style: TextStyle(color: Colors.white)),
                        ),
                      ),
      ...
      builder: (context, constraints) {
        ...
        switch (_activeTab) {
          ...
          default: // dashboard
            ...
            print(
                'DEBUG: 🎯 Rendering Dashboard - screenWidth=$screenWidth, isNarrow=$isNarrow, userName=$_userName');
